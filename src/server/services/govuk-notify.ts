...
export async function sendAnnualReviewPostEmail(
  ...
  try {
    ...
    await getNotifyClient().sendEmail(notifyTemplates[daysBeforeAnnualReviewStart], emailAddress, {
      personalisation: {
        typePlural,
        country,
        annualReviewDate,
...
export async function sendAnnualReviewProviderEmail(
  ...
  try {
    ...
    await getNotifyClient().sendEmail(GOVUK_NOTIFY_ANNUAL_REVIEW_PROVIDER_STARTED, emailAddress, {
      personalisation: {
        contactName,
        typePlural,
        country,
...
export async function sendUnpublishedProviderEmail(
  ...
  try {
    ...
    await getNotifyClient().sendEmail(notifyTemplates[daysUntilUnpublished], emailAddress, {
      personalisation: {
        contactName,
        typePlural,
        country,

...
const govUKAssetsFolderPath = path.join(NODE_MODULES, "govuk-frontend", "dist", "govuk", "assets");
const govUKFrontendMinJSPath = path.join(NODE_MODULES, "govuk-frontend", "dist", "govuk", "govuk-frontend.min.js");
const accessibleAutoCompletePath = path.join(NODE_MODULES, "accessible-autocomplete", "dist");
...
export const configureStaticServer = (server: Express): void => {
  ...
  server.use("/assets/govuk-frontend/govuk-frontend.min.js", express.static(govUKFrontendMinJSPath));

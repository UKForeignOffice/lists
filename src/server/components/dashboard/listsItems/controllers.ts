...
import { findListItemById, deleteListItem, togglerListItemIsPublished, update } from "server/models/listItem/listItem";
...
export async function listItemEditRequestValidation(req: Request, res: Response, next: NextFunction): Promise<void> {
  ...
  if (list === undefined) {
    ...
    return res.status(403).render("errors/list-management-unauthorised");
  }
  return next();
}

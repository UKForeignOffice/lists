...
async function addAuditEventForPostEmail(context: UnpublishedDateContext, list: List) {
  const postEvents: Record<number, EventMetaData> = {
    0: {
      auditEvent: AuditEvent.UNPUBLISH_ONE_DAY_REMINDER_POST_EMAIL_SENT,
      auditListItemEventName: "sendUnpublishedPostEmail",
    },
    1: {
      auditEvent: AuditEvent.UNPUBLISH_ONE_DAY_REMINDER_POST_EMAIL_SENT,
      auditListItemEventName: "sendOneDayUnpublishedReminderPostEmail",
    },
    7: {
      auditEvent: AuditEvent.UNPUBLISH_WEEKLY_REMINDER_POST_EMAIL_SENT,
      auditListItemEventName: "sendWeeklyUnpublishedReminderPostEmail",
    },
  };
  await recordListItemEvent(
    {
      eventName: postEvents[context.daysBeforeUnpublished].auditListItemEventName,
      itemId: list.id,
    },
    postEvents[context.daysBeforeUnpublished].auditEvent as AuditEvent,
    "list"
  );
}
